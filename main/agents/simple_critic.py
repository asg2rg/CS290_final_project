...
class Critic(nn.Module):
    def __init__(self, obs_dim, action_dim):
        ...
        critic_shape = obs_dim - 2 + 8 - 3*configs.STACK_SZ + 8*configs.STACK_SZ + action_dim
    ...
    def forward(self, obs, action):
        ...
        state_in = obs[:, 2:2 + 3*configs.STACK_SZ]
        ...
        critic_in = torch.cat([tgt_emb, state_emb, obs[:, 2 + 3*configs.STACK_SZ:], action], dim=1)
        # print(f"tgt emb shape: {tgt_emb.shape}, state emb shape: {state_emb.shape}, critic_in shape: {critic_in.shape}")
        return self.network(critic_in) * action_scale# if not configs.CLAMP else self.network(critic_in)