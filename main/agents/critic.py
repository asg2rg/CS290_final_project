...
class Critic(nn.Module):
    def __init__(self, obs_dim, action_dim):
        ...
        critic_shape = 8 + 8 + 8*configs.STACK_SZ + 8*configs.STACK_SZ + 8*(configs.STACK_SZ -1) + 8

        self.network = nn.Sequential(
            nn.Linear(critic_shape, 256),
            nn.ReLU(),
            nn.Linear(256, 128),
            nn.LayerNorm(128),
            nn.ReLU(),
    ...
    def forward(self, obs, action):
        ...
        action_in = action[:, :self.action_dim]
        action_emb = self.action_net(action_in)

        critic_in = torch.cat([tgt_emb, pos_emb, state_emb, agents_emb, car_act_emb, action_emb], dim=-1)
